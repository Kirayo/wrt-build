...
remove_unwanted_packages() {
    ...
    if [ -d "$BUILD_PATH/target/linux/qualcommax/base-files/etc/uci-defaults" ]; then
        find "$BUILD_PATH/target/linux/qualcommax/base-files/etc/uci-defaults/" -type f -name "99*.sh" -exec rm -f {} +
    fi
}
...
update_lucky() {
    ...
    local patch_line="\\t[ -f \$(TOPDIR)/../core/patches/lucky_${version}_Linux_\$(LUCKY_ARCH)_wanji.tar.gz ] && install -Dm644 \$(TOPDIR)/../core/patches/lucky_${version}_Linux_\$(LUCKY_ARCH)_wanji.tar.gz \$(PKG_BUILD_DIR)/\$(PKG_NAME)_\$(PKG_VERSION)_Linux_\$(LUCKY_ARCH).tar.gz"
...
remove_attendedsysupgrade() {
    find "$BUILD_PATH/feeds/luci/collections" -name "Makefile" | while read -r makefile; do
...
remove_tweaked_packages() {
    local target_mk="$BUILD_PATH/include/target.mk"
    if [ -f "$target_mk" ]; then
